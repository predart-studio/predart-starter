...
The shared language for talking about motion. Use these exact terms in code comments,
commit messages, PR descriptions, design notes, and conversations with the user. Naming a
gesture precisely ("this is a *crossfade*, not a *shared element transition*") prevents the
ambiguity that produces hectic, inconsistent motion.

> Source: the canonical glossary at https://animations.dev/vocabulary — reproduced here so the
> definitions are available offline and bound to this project's system. Definitions are quoted
> from that source; the **Use in this project** notes are local guidance.

## How to use this file

- When picking an effect, **name the gesture first**, then implement it. The name implies the
  defaults (an *ease-out* entrance, a *subtle exit*, a *scroll reveal* at `top 85%`).
- When reviewing motion, flag any gesture that is unnamed or misnamed — that is usually where
  inconsistency hides.
- Map every term to the project's tokens in `this-project-motion-system.md`. The vocabulary is
  the *what*; the system is the *how much*.
...
**Use in this project:** the house entrance is *rise + fade* (translateY + opacity, optionally
+ blur), ease-out, at the `rise-md` distance. Exits are *subtle* — smaller travel, faster, ease-in.
...
**Use in this project:** stagger off the canonical `stagger-*` scale; orchestrate with GSAP
timeline position offsets (not piles of ad-hoc `delay:` values). One rhythm per section.
...
- **Origin-aware animation** — Element animates out of its trigger, like a popover growing from the button that opened it.

**Use in this project:** animate `transform` and `opacity` only for anything that moves
continuously. `translate`, `scale`, `rotate` are compositor-friendly; `width`/`height`/`top`/`left` are not.
...
- **Direction-aware transition** — Content slides one way going forward and opposite way going back.

**Use in this project:** prefer crossfade and layout animation over hard cuts. Use `ease-inout`
for A→B moves of elements already on screen.
...
**Use in this project:** *scroll reveals* are one-shot (`once: true`, `start: "top 85%"`).
*Scroll-driven* (scrubbed) scenes use `ease: "none"` and ONE canonical scrub weight. Don't drive
animation from raw scroll events — use ScrollTrigger over Lenis.
...
**Use in this project:** hover/press are *interruptible CSS transitions* at the `micro`
duration. Press = `scale(0.96)`, always `motion-safe`-gated.
...
- **Asymmetric easing** — A curve that accelerates and decelerates at different rates. Feels more alive than symmetric.

**Use in this project:** ease-out is the default (the `house` curve). Ease-in for exits only.
Ease-in-out for on-screen A→B. Linear ONLY for scrub and marquee.
...
**Use in this project:** this site is **duration + cubic-bezier**, not spring-based — keep it
that way for cohesion. If a spring is ever used (Framer), set `bounce: 0` unless a *pop* is
explicitly wanted.
...
**Use in this project:** at most ONE ambient loop competing for attention at a time. Marquees use
`ease: "none"` and live *behind* content, never on top of an active reveal.
...
**Use in this project:** blur ≤ 8px and only on small surfaces for short, one-time effects.
Number tickers use `tabular-nums` to prevent layout shift.
...
- **will-change** — A CSS hint that an element is about to animate, so the browser can promote it.
- **Layout thrashing** — Animating properties like width, height, top, or left that force the browser to recalculate layout.

**Use in this project:** composite-only (`transform`/`opacity`) for motion; `will-change`
surgically and temporarily; never animate layout properties on large surfaces.
...
**Use in this project:** *Frequency of use* and *Purposeful animation* are the two principles
that most directly fix "hectic." Anything a user sees on every scroll should be short and subtle;
anything decorative should justify itself or be cut.
